...
type GameMatchRule struct {
	...
	GameId                int    `json:"game_id" form:"game_id" db:"define:int;comment:游戏关联ID;defaultValue:0"`                                                                       //类型,1service 2frontend 3backend 4app 5 unity 6 cocos
	Status                int    `json:"status" form:"status" db:"define:tinyint(1);comment:状态1正常2关闭;defaultValue:0"`                                                                //状态1正常2关闭
	MatchTimeout          int    `json:"match_timeout" form:"match_timeout" db:"define:int;comment:匹配超时时间(秒);defaultValue:0"`                                                        //状态1正常2关闭
	SuccessTimeout        int    `json:"success_timeout" form:"success_timeout" db:"define:int;comment:匹配成功后，对方未接收超时时间(秒);defaultValue:0"`                                           //状态1正常2关闭
	...
	TeamMaxPeople         int    `json:"team_max_people" form:"team_max_people" db:"define:int;comment:队伍最大人数;defaultValue:0"`                                                       //多人组队一起玩游戏，一个队伍最大人数
