...
type MailLog struct {
	...
	Receiver   int    `json:"receiver" db:"define:varchar(50);comment:接收者uid;defaultValue:''"`        //接收者uid
	...
	AuthStatus int    `json:"auth_status" db:"define:tinyint(1);comment:1未使用2已使用3已超时;defaultValue:0"` //验证码状态
	...
	SendTime     int `json:"send_time" db:"define:int;comment:发送时间;defaultValue:0"`             //发送时间
}
