...
type Project struct {
	...
	DataEncrypt int    `json:"data_encrypt" form:"data_encrypt" db:"define:tinyint(1);comment:传输的数据加密类型;defaultValue:0"`           //数据传输的加密类型0关闭1base64
	AuthSign    int    `json:"auth_sign" form:"auth_sign" db:"define:tinyint(1);comment:传输的数据加密类型;defaultValue:0"`                 //验证数据的签名0关闭1开启验证
