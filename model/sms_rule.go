...
type SmsRule struct {
	...
	Channel           int    `json:"channel" db:"define:tinyint(1);comment:1阿里2腾讯;defaultValue:0"`                 //渠道，1阿里2腾讯,仅支持阿里
	ThirdTemplateId   string `json:"third_template_id" db:"define:varchar(100);comment:3方模板ID;defaultValue:''"`    //3方模板ID
	ThirdBackInfo     string `json:"third_back_info" db:"define:varchar(255);comment:请示3方返回结果集;defaultValue:''"`   //3方审核返回结果集
	ThirdStatus       int    `json:"third_status" db:"define:tinyint(1);comment:3方状态;defaultValue:0"`              //3方审核状态
