...
// 	return "rsync -avz --progress --port=873 "
// }
...
func (deploy *Deploy) Publish(id int, deployTargetType int) error {
	...
	if service.Type == model.PROJECT_TYPE_SERVICE {
		...
		syncCodeShellCommand := GetRsyncCommand(rsyncInstance.Host, rsyncInstance.Port, rsyncInstance.User, "", rsyncInstance.Ext+"/"+serviceDeployConfig.Name, "", serviceDeployConfig.FullPath+"/"+serviceDeployConfig.MasterDirName)
...
var ThirdInstance = []string{"mysql", "redis", "log", "email", "etcd", "rabbitmq", "kafka", "alert", "cdn", "consul", "sms", "prometheus", "es", "kibana", "grafana", "push_gateway", "http", "domain", "oss", "grpc", "gateway", "agora", "super_visor", "rsync"}
