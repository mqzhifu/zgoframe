...
func (queueSuccess *QueueSuccess) getRedisPrefixKey() string {
	return service.RedisPrefix + service.RedisSeparation + "success"
}

//不同的匹配池(规则)，要有不同的KEY
func (queueSuccess *QueueSuccess) getRedisCatePrefixKey() string {
	return queueSuccess.getRedisPrefixKey() + service.RedisSeparation + queueSuccess.Rule.CategoryKey
}
func (queueSuccess *QueueSuccess) getRedisKeyResultPrefix() string {
	return queueSuccess.getRedisCatePrefixKey() + service.RedisSeparation + "result"
}

func (queueSuccess *QueueSuccess) getRedisKeyResult(successId int) string {
	return queueSuccess.getRedisKeyResultPrefix() + service.RedisSeparation + strconv.Itoa(successId)
}

func (queueSuccess *QueueSuccess) getRedisKeyTimeout() string {
	return queueSuccess.getRedisCatePrefixKey() + service.RedisSeparation + "timeout"
}

func (queueSuccess *QueueSuccess) getRedisResultIncKey() string {
	return queueSuccess.getRedisCatePrefixKey() + service.RedisSeparation + "inc_id"
}

//最简单的string：一个组的详细信息
func (queueSuccess *QueueSuccess) getRedisKeyGroupPrefix() string {
	return queueSuccess.getRedisCatePrefixKey() + service.RedisSeparation + "group"
}
func (queueSuccess *QueueSuccess) getRedisKeyGroup(groupId int) string {
	return queueSuccess.getRedisKeyGroupPrefix() + service.RedisSeparation + strconv.Itoa(groupId)
}
...
func (queueSuccess *QueueSuccess) strToStruct(redisStr string) Result {
	...
	//fmt.Printf("%+v",strArr)
	Teams := strings.Split(strArr[5], service.IdsSeparation)
	PlayerIds := strings.Split(strArr[6], service.IdsSeparation)
	GroupIds := strings.Split(strArr[7], service.IdsSeparation)
	result := Result{
...
func (queueSuccess *QueueSuccess) structToStr(result Result) string {
	...
	TeamsStr := util.ArrCoverStr(result.Teams, service.IdsSeparation)
	PlayerIds := util.ArrCoverStr(result.PlayerIds, service.IdsSeparation)
	GroupIds := util.ArrCoverStr(result.GroupIds, service.IdsSeparation)

	content :=
		strconv.Itoa(result.Id) + service.Separation +
			strconv.Itoa(result.RuleId) + service.Separation +
			result.MatchCode + service.Separation +
			strconv.Itoa(result.ATime) + service.Separation +
			strconv.Itoa(result.Timeout) + service.Separation +
			TeamsStr + service.Separation +
			PlayerIds + service.Separation +
			GroupIds + service.Separation +
			strconv.Itoa(result.PushId) + service.Separation
