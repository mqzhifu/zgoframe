...
func NewSendEmail(gorm *gorm.DB, myEmail *util.MyEmail) *SendEmail {
...
func (SendEmail *SendEmail) Send(projectId int, info request.SendEmail) (recordNewId int, err error) {
	...
	if rule.Type == model.EMAIL_TYPE_AUTHCODE {
		...
		//状态
		emailLog.AuthStatus = AUTH_CODE_STATUS_NORMAL
		//把刚刚生成的code替换到内容中
	...
	//创建记录之前，先更新一下已失效的记录
	SendEmail.CheckExpireAndUpStatus()
	err = SendEmail.Gorm.Create(&emailLog).Error
	if err != nil {
		return 0, errors.New("gorm err:" + err.Error())
	}

	util.MyPrint("emailLog new id:", emailLog.Id, " content:", emailLog.Content)
	SendEmail.MyEmail.SendOneEmailAsync(info.Receiver, emailLog.Title, emailLog.Content)
...
func (SendEmail *SendEmail) CheckExpireAndUpStatus() {
	var emailLog model.EmailLog
	now := util.GetNowTimeSecondToInt()
	upRsObj := SendEmail.Gorm.Model(&emailLog).Where("expire_time > 0  and expire_time <=  ? and status = ?  ", now, AUTH_CODE_STATUS_NORMAL).Update("status", AUTH_CODE_STATUS_EXPIRE)
...
func (SendEmail *SendEmail) CheckLimiterPeriod(rule model.EmailRule, receiver string) error {
	...
	err := SendEmail.Gorm.Model(model.EmailLog{}).Where(where, nowBefore, now, receiver, rule.Id).Count(&count).Error
	if err != nil {
...
func (SendEmail *SendEmail) CheckLimiterDay(rule model.EmailRule, receiver string) error {
	...
	where := "created_at >= ?  and  created_at <= ?  and receiver = ? and rule_id = ?"
	err := SendEmail.Gorm.Model(model.EmailLog{}).Where(where, start, end, receiver, rule.Id).Count(&count).Error
	if err != nil {
		if !errors.Is(err, gorm.ErrRecordNotFound) {
			return errors.New("gorm err:" + err.Error())
		}
...
//对 短信 验证码 进行校验
func (SendEmail *SendEmail) Verify(ruleId int, mobile string, authCode string) error {
	...
	var rule model.EmailRule
	err := SendEmail.Gorm.First(&rule, ruleId).Error
	if err != nil {
	...
	err = SendEmail.Gorm.Where("receiver = ?  and rule_id = ? and auth_status = ? ", mobile, ruleId, AUTH_CODE_STATUS_NORMAL).First(&emailLog).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
	...
	if emailLog.ExpireTime < now {
		...
		editEmailLog.AuthStatus = AUTH_CODE_STATUS_EXPIRE
		SendEmail.Gorm.Updates(editEmailLog)
	...
	emailLogEdit.AuthStatus = AUTH_CODE_STATUS_OK
	upRsObj := SendEmail.Gorm.Updates(emailLogEdit)
	if upRsObj.Error != nil {
