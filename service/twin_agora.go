...
type RTCRoom struct {
	...
	OnlineUids        []int  //当前在线并且在房间通话的用户
	Uids              []int  //CallUid + ReceiveUids ,只是记录下，方便函数调用
}
...
func (twinAgora *TwinAgora) GetUserById(uid int) (RTCUser, bool) {
	myRTCUser, ok := twinAgora.RTCUserPool[uid]
	if !ok {
		util.MyPrint("GetUserById empty:", uid)
	}
	return myRTCUser, ok
}
