...
func (p *Pinger) recvICMP(conn *icmp.PacketConn, recv chan<- *packet, ctx *myContext, wg *sync.WaitGroup) {
	...
	for {
		...
		bytes := make([]byte, 512)
		conn.SetReadDeadline(time.Now().Add(time.Millisecond * 900))
		p.debugln("recvICMP(): ReadFrom Start")
		number, ra, err := conn.ReadFrom(bytes)
		...
		if err != nil {
			if neterr, ok := err.(*net.OpError); ok {
				if neterr.Timeout() {
					...
					ctx.err = err
					p.mu.Unlock()
					p.debugln("recvICMP(): close(ctx.done) 2")
					close(ctx.done)
					p.debugln("recvICMP(): wg.Done() 2")
					wg.Done()
					return
