...
func   (fileUpload *FileUpload)AliOSS(localFilePath string , relativePath , FileName string)error{
	...
	//localFileName := "1_1653988302.txt"
	//objectName := "/aaaa/"+localFileName
	//localFilePath := "/data/www/golang/src/zgoframe/static/upload/" + localFileName
...
func   (fileUpload *FileUpload)UploadOne(file multipart.File,header *multipart.FileHeader)(relativeFileName string ,err error){
	//fileUpload.AliOSS()
	//ExitPrint("finish.")
	...
	extName := filenameArr[1]
	extFilter := fileUpload.FilterByExtString(fileUpload.Option.Category,extName)
	if !extFilter{
	...
	MyPrint("realFileType:",realFileType)
	if realFileType != ""{
		...
		if extName != "txt"{
			return "",errors.New("文件类型非法")
		}
	}

	baseDir := fileUpload.Option.Path + "/" + fileUpload.GetHashDirName()
	_,err = PathExists(baseDir)
	if err != nil {
		if os.IsNotExist(err){
			MyPrint("dir not exist ,mkdir:"+baseDir)
		...
		MyPrint("baseDir exist:"+baseDir)
	}

	fileName := strconv.Itoa(fileUpload.Option.Category) + "_" + strconv.Itoa(GetNowTimeSecondToInt()) + "." +extName
	relativeFileName =  fileUpload.GetHashDirName() + "/" + fileName
	newFileName := fileUpload.Option.Path + "/" +relativeFileName
	//ExitPrint(newFileName)
	...
	err = fileUpload.AliOSS(newFileName,fileUpload.GetHashDirName(),fileName)
	if err != nil{
		return "",err
	}
...
func   (fileUpload *FileUpload)GetAllowFileTypeList(category int)(rs []string,err error){
	...
	docs := []string{"txt","doc","docx","dotx","json","cvs","xls","xlsx","sql","msword","pptx","pptx"}
